...
`uv.lock` is generated by the real resolver and includes package sources/hashes. Python 3.11 is retained. Frozen installation succeeded on macOS ARM64 and in the Docker Linux ARM64 build. CI separately evaluates Linux x86_64 after publication; local ARM64 success is not an x86_64 or Orange Pi claim.
