...
Software already present: FastAPI/Pydantic application, separate durable worker, CrewAI planner/reviewer adapter, Ollama and compatible HTTP paths, Qdrant document retrieval from text PDF, DOCX and plain text with protected page/paragraph citations, catalog-based Decimal quotations exported to DOCX/PDF, approved individually assigned local tasks and deadline tracking, deterministic financial calculations and lineage, audit and an English/Russian dashboard. The default demo uses clearly labeled deterministic responses and synthetic examples; users can also import their own documents and catalogs for a controlled demonstration. No real-customer deployment or production data-handling certification is claimed. Validation details and known dependency findings are in VALIDATION.md and DEPENDENCY_REVIEW.md.
